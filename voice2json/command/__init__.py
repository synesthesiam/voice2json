...
class WebRtcVadRecorder(VoiceCommandRecorder):
    def __init__(
        ...
        self.before_buffers = int(math.ceil(self.before_seconds / self.seconds_per_buffer))

        # Pre-compute values
        self.speech_buffers = int(math.ceil(self.speech_seconds / self.seconds_per_buffer))

    def record(audio_source: BinaryIO) -> VoiceCommand:
        audio_chunks = Queue()
